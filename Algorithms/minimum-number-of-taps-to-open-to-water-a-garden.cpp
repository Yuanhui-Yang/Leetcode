...
public:
    int minTaps(int n, vector<int>& ranges) {
        ...
        int i = 0;
        int sz = A.size();
        int boundary = 0; // left to cover
        int result = 0;
        while (i < sz and boundary < n)
        {
            ++result;
            int j = i;
            int rboundary = i; // right covered
            while (j < sz and A[j][0] <= boundary)
            {
                rboundary = max(rboundary, A[j][1]);
                ++j;
            }
            if (i == j)
            {
            ...
            i = j;
            //cout << '[' << boundary << ',' << rboundary << ']' << endl;
            boundary = rboundary;
        }
        return boundary < n ? -1 : result;
    }
};
// @lc code=end