...
public:
    int minDifficulty(vector<int>& jobDifficulty, int d) {
        ...
        vector<vector<int>> A(d + 1, vector<int>(sz + 1, 0));
        for (int i = 1; i <= d; ++i)
        {
            A[i][i] = A[i - 1][i - 1] + jobDifficulty[i - 1];
        }
        for (int i = 2; i <= sz; ++i)
        {
            A[1][i] = max(A[1][i - 1], jobDifficulty[i - 1]);
        }
        for (int i = 2; i <= d; ++i)
        {
            for (int j = i + 1; j <= sz; ++j)
            {
                ...
                {
                    int curr = jobDifficulty[k];
                    global_max = max(global_max, curr);
                    int tmp = A[i - 1][k] + global_max;
                    val = min(val, tmp);
                }
            }
        }
