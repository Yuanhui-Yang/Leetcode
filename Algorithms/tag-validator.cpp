...
public:
	bool isValid(string code) {
		...
		for (int i = 0, n = code.size(); i < n; ++i) {
			...
			if (i + 9 <= n and code.substr(i, 9) == "<![CDATA[") {
				if (stk.empty()) {
					return false;
				}
				i += 9;
				while (i + 3 <= n and code.substr(i, 3) != "]]>") {
					++i;
				}
				if (i + 3 > n) {
					return false;
			...
			if (i + 2 <= n and code.substr(i, 2) == "</") {
				i += 2;
				int j = i;
				while (i + 1 <= n and code[i] != '>') {
					if (isupper(code[i])) {
						++i;
					}
					else {
				...
				if (i + 1 > n or stk.empty() or (stk.top() != code.substr(j, i - j))) {
					return false;
				}
				stk.pop();
				if (stk.empty()) {
